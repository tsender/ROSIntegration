...
// #endif
	...
	{
		UE_LOG(LogROSWebSockets, Verbose, TEXT("Failed to create our SSL context, this will result in libwebsockets managing its own SSL context, which calls SSLs global cleanup functions, impacting other uses of SSL"));
	}
	...
	{
		UE_LOG(LogROSWebSockets, Error, TEXT("Failed to initialize libwebsockets"));
		return;
	}
	...
	if (!Thread)
	{
		UE_LOG(LogROSWebSockets, Error, TEXT("FLwsWebSocketsManager failed to initialize thread!"));
...
static void LwsLog(int Level, const char* LogLine)
{
	...
	if (bIsWarning || UE_LOG_ACTIVE(LogROSWebSockets, Verbose))
	{
		...
		if (bIsWarning)
		{
			UE_LOG(LogROSWebSockets, Warning, TEXT("Lws(%s): %s"), LwsLogLevelToString(Level), *ConvertedLogLine);
		}
		else
		{
			UE_LOG(LogROSWebSockets, Verbose, TEXT("Lws(%s): %s"), LwsLogLevelToString(Level), *ConvertedLogLine);
		}
	}
}
...
{
	return FLwsWebSocketsManager::Get()->CallbackWrapper(Connection, Reason, UserData, Data, Length);
}
