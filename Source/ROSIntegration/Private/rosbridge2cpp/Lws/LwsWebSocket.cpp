...
{
	UE_LOG(LogROSWebSockets, VeryVerbose, TEXT("FLwsWebSocket[%d]: Constructed url=%s protocols=%s"), Identifier, *InUrl, *FString::Join(Protocols, TEXT(",")));
}

FLwsWebSocket::~FLwsWebSocket()
{
	UE_LOG(LogROSWebSockets, VeryVerbose, TEXT("FLwsWebSocket[%d]: Destroyed"), Identifier);
...
void FLwsWebSocket::Connect()
{
	...
	{
		UE_LOG(LogROSWebSockets, Warning, TEXT("FLwsWebSocket[%d]::Connect: State is not None (%s), unable to start connecting!"), Identifier, ToString(LastGameThreadState));
		return;
	}
	...
	if (!bDisableDomainWhitelist)
	{
		...
		{
			State = EState::Error;
			UE_LOG(LogROSWebSockets, Warning, TEXT("FLwsWebSocket[%d]::Connect: %s is not whitelisted. Refusing to connect."), Identifier, *Url);
	...
	{
		UE_LOG(LogROSWebSockets, Log, TEXT("FLwsWebSocket[%d]::Connect: Domain whitelisting has been disabled by config."), Identifier);
	}
	...
	UE_LOG(LogROSWebSockets, Verbose, TEXT("FLwsWebSocket[%d]::Connect: setting State=%s url=%s bWantsMessageEvents=%d bWantsRawMessageEvents=%d"), Identifier, ToString(State), *Url, (int32)bWantsMessageEvents, (int32)bWantsRawMessageEvents);

	// Clear up any data from previous runs
	ClearData();

	FLwsWebSocketsManager::Get()->StartProcessingWebSocket(this);
}
...
void FLwsWebSocket::Close(int32 Code, const FString& Reason)
{
	...
	{
		UE_LOG(LogROSWebSockets, Warning, TEXT("FLwsWebSocket[%d]::Close: Already closing, ignoring subsequent attempt"), Identifier);
		return;
	}
	...
	UE_LOG(LogROSWebSockets, Verbose, TEXT("FLwsWebSocket[%d]::Close: Close queued with code=%d reason=%s"), Identifier, Code, *Reason);
...
void FLwsWebSocket::SendFromQueue()
{
	...
	if (SendQueue.Peek(CurrentBuffer))
	{
		...
		if (!bWriteSuccessful)
		{
			...
			UE_LOG(LogROSWebSockets, Warning, TEXT("FLwsWebSocket[%d]::SendFromQueue: Error writing buffer Size=%d BytesWritten=%d bIsBinary=%d"), 
		...
		{
			UE_LOG(LogROSWebSockets, VeryVerbose, TEXT("FLwsWebSocket[%d]::SendFromQueue: Wrote %d bytes, %d bytes remaining in this packet"), 
...
int FLwsWebSocket::LwsCallback(lws* Instance, lws_callback_reasons Reason, void* Data, size_t Length)
{
	...
	case LWS_CALLBACK_CLIENT_ESTABLISHED:
	{
		...
		UE_LOG(LogROSWebSockets, Verbose, TEXT("FLwsWebSocket[%d]::LwsCallback: Received LWS_CALLBACK_CLIENT_ESTABLISHED, setting State=%s PreviousState=%s"), Identifier, ToString(State), ToString(PreviousState));
	...
	case LWS_CALLBACK_CLIENT_RECEIVE:
	{
		...
		UE_LOG(LogROSWebSockets, VeryVerbose, TEXT("FLwsWebSocket[%d]::LwsCallback: Received LWS_CALLBACK_CLIENT_RECEIVE Length=%d BytesLeft=%d"), Identifier, Length, BytesLeft);
	...
	case LWS_CALLBACK_WS_PEER_INITIATED_CLOSE:
	{
			...
			State == EState::ClosingByRequest)
		{
			...
			UE_LOG(LogROSWebSockets, Verbose, TEXT("FLwsWebSocket[%d]::LwsCallback: Received LWS_CALLBACK_WS_PEER_INITIATED_CLOSE, setting State=%s CloseStatus=%d Reason=%s bPeerSpecifiedReason=%d PreviousState=%s"),
		...
		{
			UE_LOG(LogROSWebSockets, Verbose, TEXT("FLwsWebSocket[%d]::LwsCallback: Received LWS_CALLBACK_WS_PEER_INITIATED_CLOSE, but ignoring because our State=%s CloseStatus=%d Reason=%s"),
	...
	case LWS_CALLBACK_WSI_DESTROY:
		...
		if (State == EState::Connecting)
		{
			...
			UE_LOG(LogROSWebSockets, Verbose, TEXT("FLwsWebSocket[%d]::LwsCallback: Received LWS_CALLBACK_WSI_DESTROY, setting State=%s PreviousState=%s"), Identifier, ToString(EState::Error), ToString(State));
		...
		else
		{
			...
			UE_LOG(LogROSWebSockets, Verbose, TEXT("FLwsWebSocket[%d]::LwsCallback: Received LWS_CALLBACK_WSI_DESTROY, State=%s"), Identifier, ToString(State));
		}
		LwsConnection = nullptr;
		break;
	case LWS_CALLBACK_CLOSED:
	{
			...
			State == EState::ClosingByRequest)
		{
			...
			UE_LOG(LogROSWebSockets, Verbose, TEXT("FLwsWebSocket[%d]::LwsCallback: Received LWS_CALLBACK_CLOSED, setting State=%s CloseReason=%s PreviousState=%s"),
		...
		{
			UE_LOG(LogROSWebSockets, Verbose, TEXT("FLwsWebSocket[%d]::LwsCallback: Received LWS_CALLBACK_CLOSED, ignoring and waiting for LWS_CALLBACK_WSI_DESTROY, State=%s"),
	...
	case LWS_CALLBACK_CLIENT_CONNECTION_ERROR:
	{
		...
		bool bWakeGameThread = false;
		UE_LOG(LogROSWebSockets, Verbose, TEXT("FLwsWebSocket[%d]::LwsCallback: Received LWS_CALLBACK_CLIENT_CONNECTION_ERROR, setting State=%s CloseReason=%s PreviousState=%s"),
...
void FLwsWebSocket::GameThreadFinalize()
{
	...
	UE_LOG(LogROSWebSockets, Verbose, TEXT("FLwsWebSocket[%d]::GameThreadFinalize: setting State=%s PreviousState=%s"),
...
void FLwsWebSocket::LwsThreadTick()
{
	if (State == EState::Connected)
	{
		...
		if (State == EState::ClosingByRequest)
		{
			UE_LOG(LogROSWebSockets, Verbose, TEXT("FLwsWebSocket[%d]::LwsThreadTick: Close requested while connected, setting State=%s PreviousState=%s"),
...
void FLwsWebSocket::ConnectInternal(struct lws_context &LwsContext)
{
	...
	UE_LOG(LogROSWebSockets, Verbose, TEXT("FLwsWebSocket[%d]::ConnectInternal: setting State=%s PreviousState=%s"),
	...
	if (lws_parse_uri((char*)UrlUtf8.Get(), &UrlProtocol, &ParsedAddress, &ParsedPort, &TmpUrlPath))
	{
		...
		UE_LOG(LogROSWebSockets, Verbose, TEXT("FLwsWebSocket[%d]::ConnectInternal: setting State=%s PreviousState=%s Reason=%s"),
	...
	{
		UE_LOG(LogROSWebSockets, Verbose, TEXT("FLwsWebSocket[%d]::ConnectInternal: lws_client_connect_via_info succeeded"), Identifier);
	}
	else
	{
